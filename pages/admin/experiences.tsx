...
export default function AdminExperiences() {
  ...
  const { register, handleSubmit, reset, setValue, watch } = useForm({
    ...
    defaultValues: {
      company: "", start: "", end: "", tools: [] as string[],
      translations: { en: { role: "", description: "" }, id: { role: "", description: "" } },
      published: true,
    } as any,
  });
  ...
  const onSubmit = async (values: any) => {
    ...
    if (res.ok) { reset(); mutate(); alert("Created"); } else { alert("Failed"); }
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
        <section className={card}>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div><p className={label}>Company</p><input className={input} {...register("company")} /></div>
            <div><p className={label}>Start</p><input type="date" className={input} {...register("start")} /></div>
            <div><p className={label}>End (optional)</p><input type="date" className={input} {...register("end")} /></div>
          </div>
          <div className="mt-3">
        ...
        <section className={card}>
          ...
          <input className={input} placeholder="Role (EN)" {...register("translations.en.role")} />
          <textarea className={input} rows={3} placeholder="Description (EN)" {...register("translations.en.description")} />
        </section>

        <section className={card}>
          <h3 className="mb-3 font-medium">Konten (ID)</h3>
          <input className={input} placeholder="Peran (ID)" {...register("translations.id.role")} />
          <textarea className={input} rows={3} placeholder="Deskripsi (ID)" {...register("translations.id.description")} />
        </section>

        <div className="flex justify-center"><button className={btn}>Simpan</button></div>
      </form>
          ...
          {data?.data?.map((e: any) => (
            <li key={e.id} className={card}>
              ...
              <p className="text-xs text-white/60">
                {new Date(e.start).toLocaleDateString()} {e.end ? `– ${new Date(e.end).toLocaleDateString()}` : "– Present"} • {e.tools?.join(", ")}
              </p>
            </li>
          ))}
        </ul>
