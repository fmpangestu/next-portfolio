...
export default function AdminCertificates() {
  ...
  const { register, handleSubmit, reset, setValue } = useForm({
    ...
    defaultValues: {
      image_path: "",
      sertificate_url: "",
      translations: { en: { title: "", description: "" }, id: { title: "", description: "" } },
      published: true,
    } as any,
  });
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
        <section className={card}>
          ...
          <FileUpload onUploaded={(p) => setValue("image_path", p, { shouldDirty: true })} />
        ...
        <section className={card}>
          ...
          <input className={input} placeholder="https://..." {...register("sertificate_url")} />
        </section>

        <section className={card}>
          <h3 className="mb-3 font-medium">Content (EN)</h3>
          <input className={input} placeholder="Title (EN)" {...register("translations.en.title")} />
        ...
        <section className={card}>
          ...
          <input className={input} placeholder="Judul (ID)" {...register("translations.id.title")} />
          ...
          {data?.data?.map((c: any) => (
            <li key={c.id} className={card}>
              <div className="flex items-center gap-3">
                {c.image_path && <img src={c.image_path} className="h-10 w-10 rounded object-cover" alt="" />}
                <div>
                  <p className="font-medium">{c.title}</p>
                  {c.sertificate_url && <a className="text-xs underline" href={c.sertificate_url} target="_blank" rel="noreferrer">Visit</a>}
                </div>
              </div>
            </li>
          ))}
